...
export default function GalleryPage() {
  ...
  const scrollToCategory = (catKey) => {
    setActiveCategory(catKey);
    const el = document.getElementById(`section-${catKey}`);
    if (el) {
      const yOffset = -140; // Account for fixed main header + sticky floating filter
      const y = el.getBoundingClientRect().top + window.pageYOffset + yOffset;
      window.scrollTo({ top: y, behavior: "smooth" });
    }
  };

  // ScrollSpy to highlight active category in sticky bar automatically
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    ...
    >
      {/* 1. HERO SECTION */}
      <section className="relative w-full h-[60vh] md:h-[70vh] flex items-end pb-20 md:pb-24">
        <Image
      ...
      {/* 2. FLOATING / STICKY NAVBAR FILTER */}
      <div
        className={`sticky top-14 md:top-16 lg:top-[80px] z-40 transition-all duration-300 backdrop-blur-xl border-b shadow-md ${isDark
          ? "bg-[#010e22]/90 border-[#FCD57B]/20"
          : "bg-white/90 border-black/10"
          }`}
      >
        <div className="max-w-[1380px] mx-auto px-4 md:px-6">
          <div className="flex items-center justify-start md:justify-center overflow-x-auto no-scrollbar gap-2 md:gap-8 py-3.5">
            {GALLERY_CATEGORIES.map((cat) => {
              ...
              return (
                <button
                  ...
                  // className={`shrink-0 text-xs font-medium tracking-[0.18em] uppercase transition-all duration-300 relative py-1.5 px-3 rounded-full flex items-center gap-2 ${isActive
                  className={`shrink-0 text-xs font-medium tracking-[0.18em] uppercase transition-all duration-300 relative py-1.5 px-3 flex items-center gap-2 ${isActive
                    ? isDark
                      ? "text-[#FCD57B] font-bold"
                      : "text-[#8B6B2E] font-bold"
                    : isDark
                      ? "text-white/70 hover:text-white"
                      : "text-[#011434]/70 hover:text-[#011434]"
                    }`}
                >
                  <span>{cat.label[lang]}</span>
                  <span
                    // className={`text-[9px] px-1.5 py-0.2 rounded-full font-mono ${isActive
                    className={`text-[9px] px-1.5 py-0.2 font-mono ${isActive
                      ? isDark
                        ? "bg-[#FCD57B]/20 text-[#FCD57B]"
                        : "bg-[#8B6B2E]/15 text-[#8B6B2E]"
                      : isDark
                        ? "bg-white/10 text-white/50"
                        : "bg-black/10 text-[#011434]/50"
                      }`}
                  >
                  </span>
                  {isActive && (
                    <span
                      style={{
                        backgroundColor: isDark ? "#FCD57B" : "#8B6B2E",
                      }}
      ...
      {/* 3. CONTINUOUS CATEGORY SECTIONS */}
      <div className="w-full max-w-[1380px] mx-auto px-6 pt-12 pb-28">
        {GALLERY_CATEGORIES.map((cat) => {
          ...
          return (
            <section
              key={cat.key}
              id={`section-${cat.key}`}
              className="pt-12 md:pt-16 mb-20 scroll-mt-36"
            >
              ...
              <div className="text-center mb-12">
                ...
                <h2
                  style={{ fontFamily: "var(--font-cormorant-garamond)" }}
                  className="text-3xl md:text-4xl font-semibold tracking-widest leading-none uppercase mb-4"
                >
                  {cat.label[lang]}
                </h2>
                <div
                  style={{ backgroundColor: accentText }}
                  className="w-12 h-[1.5px] mx-auto opacity-70"
                />
              </div>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:gap-8 lg:grid-cols-3">
                {categoryImages.map((img) => {
                  const globalIdx = GALLERY_IMAGES.findIndex(
                    (item) => item.id === img.id
                  );
                  ...
                  return (
                    <div
                      ...
                      className={`group relative aspect-[4/3] cursor-pointer overflow-hidden border shadow-lg transition-all duration-500 hover:-translate-y-1.5 ${isDark
                    ...
                    >
                      ...
                      <div className="absolute right-4 top-4 z-10 flex h-9 w-9 scale-75 items-center justify-center border border-white/20 bg-black/50 text-white opacity-0 backdrop-blur-md transition-all duration-300 group-hover:scale-100 group-hover:opacity-100">
      ...
      {currentLightboxImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-8 bg-black/90 backdrop-blur-md transition-all duration-300 animate-fadeIn">
          ...
          <button
            ...
            className="absolute top-6 right-6 z-50 p-3 bg-white/10 hover:bg-white/20 text-white transition-colors duration-300 border border-white/20"
            aria-label="Close"
          >
          ...
          <button
            onClick={handlePrev}
            className="absolute left-4 md:left-8 z-50 p-3.5 bg-black/50 hover:bg-[#FCD57B] text-white hover:text-[#011434] transition-all duration-300 border border-white/20 hover:border-[#FCD57B]"
          ...
          <button
            ...
            className="absolute right-4 md:right-8 z-50 p-3.5 bg-black/50 hover:bg-[#FCD57B] text-white hover:text-[#011434] transition-all duration-300 border border-white/20 hover:border-[#FCD57B]"
          ...
          <div className="relative max-w-5xl w-full max-h-[85vh] flex flex-col items-center justify-center">
            ...
            <div className="relative w-full h-[55vh] md:h-[68vh] overflow-hidden shadow-2xl border border-white/10">
              <Image
            ...
            <div className="mt-4 text-center max-w-2xl mx-auto px-4">
              <div className="flex items-center justify-center gap-3 mb-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-[#FCD57B] px-3 py-0.5 bg-[#FCD57B]/10 border border-[#FCD57B]/30">
                  {
